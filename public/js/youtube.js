$(document).ready(function () {
	...
	async function handleVideo(videoUrl, details) {
		try {
			...
			console.error(err);
			toastError(err.message);
			videoUrl = videoUrl.replace('www.youtube.com', 'www.ssyoutube.com');
			$('#upload-form').find('.video-drop').find('#download').html(`<a href="${videoUrl}" target="_blank"><i class="fas fa-download"></i> Download Video</a>`);
			$('#upload-form').find('.video-drop').find('#download').css('display', 'block');
			$('#upload-form').find('.video-drop').find('#download').css('margin-top', '0.75em');
			$('#upload-form').find('.video-drop').find('#download').show();
		} finally {
