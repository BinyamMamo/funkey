$(document).ready(function () {
	...
	$('#upload-form').on('submit', async function (event) {
		...
		reader.onload = async function (e) {
			...
			let vtt = data.vtt;
			console.log('vtt:', vtt);
			lyrics.file = new Blob([vtt], { type: 'text/vtt' });
			lyrics.name = `${musicName}.vtt`;
			lyrics.type = 'text/vtt';
			const videoDrop = $(this).find('.video-drop')[0];
			const lyricsDrop = $(this).find('.lyrics-drop')[0];
			console.log('thumbnail:', thumbnail);
			console.log('video:', video);
			console.log('lyrics:', lyrics);
			console.log('lyrics file:', lyrics.file);
